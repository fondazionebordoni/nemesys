...
void find_devices() {
  ...
  if (alldevs == NULL) {
    sprintf(err_str, "No Sniffable Device or User Without Root Permissions");
    err_flag =  - 1;
    return ;
  }
  ...
  for (dl = alldevs; dl; dl = dl->next) {
    ...
    if (pcap_lookupnet(dl->name, &netp, &maskp, errbuf) != 0) {
      sprintf(err_str, "LookUpNet Warnings: %s", errbuf);
      err_flag = 0;
    }
    ...
    if (dl->addresses != NULL) {
      ...
      if (IpInNet != 1) {
          ...
          while ((IpInNet != 1) && (dl->addresses->next)) {
            ...
            addr.s_addr = ((struct sockaddr_in*)(dl->addresses->addr))
  ->sin_addr.s_addr;
            ip = inet_ntoa(addr);
            IpInNet = ip_in_net(ip, device[ind_dev].net, device[ind_dev].mask);
          } 
...
void initialize(u_char *dev, int promisc, int timeout, int snaplen, int buffer) {
  ...
  //printf("\nArpinger initialize()");
  find_devices();
  //printf("\nFound %d devices", ind_dev);

  if (err_flag != 0) {
    sprintf(err_str, "\nError during find_devices()");
    return ;
  }
  
  //printf("\nSearching for device for arping");
  for (i = 1; i <= ind_dev; i++) {
    //printf("\nComparing %s, %s, %s", dev, device[i].name, device[i].ip);
    if ((strcmp(dev, device[i].name) == 0) || (strcmp(dev, device[i].ip) == 0)) {
      //printf("\nFound device: %s", device[i].name);
      num_dev = i;
    }
  }
  ...
  //printf("\npcap_create on device: %s", device[num_dev].name);
  if ((handle = pcap_create(device[num_dev].name, errbuf)) == NULL) {
    sprintf(err_str, "Couldn't open device: %s", errbuf);
    err_flag =  - 1;
    return ;
  }

  //printf("\npcap_set_promisc on device: %s", device[num_dev].name);
  if (pcap_set_promisc(handle, promisc) != 0) {
    sprintf(err_str, "PromiscuousMode error: %s", errbuf);
    err_flag =  - 1;
    return ;
  }

  //printf("\npcap_set_timeout on device: %s", device[num_dev].name);
  if (pcap_set_timeout(handle, timeout) != 0) {
    sprintf(err_str, "Timeout error: %s", errbuf);
    err_flag =  - 1;
    return ;
  }

  //printf("\npcap_set_snaplen on device: %s", device[num_dev].name);
  if (pcap_set_snaplen(handle, snaplen) != 0) {
    sprintf(err_str, "Snapshot error: %s", errbuf);
    err_flag =  - 1;
    return ;
  }

  //printf("\npcap_set_buffer_size on device: %s", device[num_dev].name);
  if (pcap_set_buffer_size(handle, buffer) != 0) {
    sprintf(err_str, "SetBuffer error: %s", errbuf);
    err_flag =  - 1;
    return ;
  }

  //printf("\npcap_activate on device: %s", device[num_dev].name);
  if (pcap_activate(handle) != 0) {
    sprintf(err_str, "Activate error: %s", errbuf);
    err_flag =  - 1;
    return ;
  }
  ...
  (pcap_datalink(handle)), pcap_datalink_val_to_description(pcap_datalink
  (handle)));
    }
  }
  //DEBUG-END
...
void setfilter(const char *filter) {
  ...
  if (pcap_compile(handle, &filterprog, filter, 0, maskp) ==  - 1) {
    sprintf(err_str, "Error in pcap_compile filter");
    err_flag =  - 1;
    return ;
  }

  if (pcap_setfilter(handle, &filterprog) ==  - 1) {
    sprintf(err_str, "Error setting filter");
    err_flag =  - 1;
    return ;
  }
...
static PyObject *arpinger_send(PyObject *self, PyObject *args) {
  ...
  if (handle != NULL) {
    if (pcap_sendpacket(handle, pkt_to_send, pkt_size) != 0) {
      sprintf(err_str, "Couldn't send the packet: %s", errbuf);
      err_flag =  - 1;
    }
  } else {
    sprintf(err_str, 
  "Couldn't send any packet: No Hadle Active on Networks Interfaces");
    err_flag =  - 1;
  }
...
static PyObject *arpinger_receive(PyObject *self) {
  ...
  if (handle != NULL) {
    ...
    switch (pkt_received) {
      ...
      default:
        ...
        py_pcap_hdr = PyString_FromStringAndSize((u_char*)pcap_hdr, sizeof(struct
  pcap_pkthdr));
    ...
    sprintf(err_str, 
  "Couldn't receive any packet: No Hadle Active on Networks Interfaces");
    err_flag =  - 1;
  ...
  return Py_BuildValue("{s:i,s:s,s:O,s:O}", "err_flag", err_flag, "err_str",
  err_str, "py_pcap_hdr", py_pcap_hdr, "py_pcap_data", py_pcap_data);
}
